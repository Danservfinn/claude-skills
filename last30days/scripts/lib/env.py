...
def get_config() -> Dict[str, Any]:
    ...
    config = {
        ...
        'ZAI_API_KEY': os.environ.get('ZAI_API_KEY') or file_env.get('ZAI_API_KEY'),
        'ZAI_BASE_URL': os.environ.get('ZAI_BASE_URL') or file_env.get('ZAI_BASE_URL', 'https://open.bigmodel.cn/api/paas/v4/chat/completions'),
        'ZAI_MODEL': os.environ.get('ZAI_MODEL') or file_env.get('ZAI_MODEL', 'glm-4.7'),
        'REDDIT_PROVIDER': os.environ.get('REDDIT_PROVIDER') or file_env.get('REDDIT_PROVIDER', ''),
...
def get_available_sources(config: Dict[str, Any]) -> str:
    ...
    Note: ZAI can be used as an alternative to OpenAI for Reddit search.
    """
    has_openai = bool(config.get('OPENAI_API_KEY'))
    has_zai = bool(config.get('ZAI_API_KEY'))
    has_xai = bool(config.get('XAI_API_KEY'))

    # ZAI serves as alternative to OpenAI for Reddit search
    has_reddit_provider = has_openai or has_zai

    if has_reddit_provider and has_xai:
        return 'both'
    elif has_reddit_provider:
        return 'reddit'
    elif has_xai:
        return 'x'
    else:
...
def get_reddit_provider(config: Dict[str, Any]) -> Optional[str]:
    """Determine which provider to use for Reddit search.

    Returns: 'openai', 'zai', or None

    Respects REDDIT_PROVIDER setting if set, otherwise prefers OpenAI.
    """
    # Check for explicit provider preference
    preferred = config.get('REDDIT_PROVIDER', '').lower()
    if preferred == 'zai' and config.get('ZAI_API_KEY'):
        return 'zai'
    elif preferred == 'openai' and config.get('OPENAI_API_KEY'):
        return 'openai'

    # Default: prefer OpenAI if available, then ZAI
    if config.get('OPENAI_API_KEY'):
        return 'openai'
    elif config.get('ZAI_API_KEY'):
        return 'zai'
    return None
...
def get_missing_keys(config: Dict[str, Any]) -> str:
    ...
    Note: ZAI can substitute for OpenAI for Reddit search.
    """
    has_openai = bool(config.get('OPENAI_API_KEY'))
    has_zai = bool(config.get('ZAI_API_KEY'))
    has_xai = bool(config.get('XAI_API_KEY'))
    ...
    if has_reddit_provider and has_xai:
        ...
        return 'reddit'  # Missing OpenAI/ZAI key
    else:
