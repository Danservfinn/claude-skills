...
# ZAI (BigModel/Zhipu AI) API - supports web_search tool
ZAI_DEFAULT_MODEL = "glm-4.7"
ZAI_FALLBACK_MODELS = ["glm-4.7", "glm-4.6", "glm-4.5"]
...
def select_zai_model(config: Dict) -> str:
    """Select the ZAI model to use.

    Args:
        config: Configuration dict with ZAI_MODEL

    Returns:
        Selected model ID (defaults to glm-4)
    """
    return config.get("ZAI_MODEL") or ZAI_DEFAULT_MODEL
...
def get_models(
    ...
    Returns:
        Dict with 'openai', 'xai', and 'zai' keys
    """
    result = {"openai": None, "xai": None, "zai": None}
    ...
    if config.get("ZAI_API_KEY"):
        result["zai"] = select_zai_model(config)
